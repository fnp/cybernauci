...
<script id="nf-tmpl-mobile-menu-button" type="text/template">
    <a class="nf-button nf-mobile-menu title=" Expand Menu" <%= maybeDisabled() %>" href="#"><span
        class="dashicons dashicons-editor-ul"></span></a>
</script>
...
<script id="nf-tmpl-app-header-view-changes" type="text/template">
    <a class="nf-cancel viewChanges" title="View Changes" style="text-decoration: none;" href="#"><span
...
<script id="nf-tmpl-main-header-fields" type="text/template">
    <input class="nf-button secondary nf-change-domain" data-domain="actions" type="button"
           value="Edit Emails and Actions"/>
</script>
...
<script id="nf-tmpl-main-content-fields-empty" type="text/template">
    <div class="nf-fields-empty">
        ...
        <p>Get started by adding your first form field. Just click the plus and select the fields you want. It’s that
            easy.</p>
    </div>
</script>
...
<script id="nf-tmpl-main-content-actions-empty" type="text/template">
    <tr>
        <td colspan="4">
            ...
            <p>Get started by adding your first form field. Just click the plus and select the actions you want. It’s
                that easy.</p>
        </td>
    </tr>
</script>
...
<script id="nf-tmpl-main-content-field" type="text/template">
    <div id="<%= getFieldID() %>" class="<%= renderClasses() %>" data-id="<%= id %>"><%= renderIcon() %><span
            class="nf-field-label"><%= label %> <%= renderRequired() %></span>
        <ul class="nf-item-controls"></ul>
    </div>
</script>

<script id="nf-tmpl-item-controls" type="text/template">
    <li class="nf-item-edit"><a href="#" title="Edit"><span
                class="nf-edit-settings dashicons dashicons-admin-generic"></span><span class="nf-item-editing">Editing <%= objectType %></span></a>
    </li>
    <li class="nf-item-duplicate"><a href="#" title="Duplicate"><span
                class="nf-duplicate dashicons dashicons-admin-page"></span></a></li>
    <li class="nf-item-delete"><a href="#" title="Delete"><span
                class="nf-delete dashicons dashicons-dismiss"></span></a></li>
</script>
...
<script id="nf-tmpl-mobile-menu-item" type="text/template">
    <li><a href="<%= renderUrl() %>" title="<%= nicename %>" tabindex="-1" target="<%= renderTarget() %>" <%=
        renderDisabled() %> ><%= renderDashicons() %><%= nicename %></a></li>
</script>
...
<script id="nf-tmpl-drawer-content-view-changes-item" type="text/template">
    ...
    <tr class="disabled-row">
        ...
        <td>
            <span class="dashicons dashicons-<%= label.dashicon %>"></span> <span
                class="nf-changes-item <%= ( disabled ) ? 'disabled' : '' %>"></span>
        </td>
        <td>
            <%= label.object %>
        </td>
...
<script id="nf-tmpl-drawer-staged-field" type="text/template">
    <span class="nf-item-dock" id="<%= id %>" data-id="<%= slug %>"><span class="fa fa-<%= icon %>"></span><%= nicename %><span
...
<script id="nf-tmpl-drawer-field-type-button" type="text/template">
    <div class="nf-field-type-button nf-field-type-draggable <%= savedField() %>" data-id="<%= id %>">
        <div class="nf-item" data-id="<%= id %>" tabindex="0"><span class="fa fa-<%= icon %>"></span><%= nicename %>
        </div>
    </div>
</script>
...
<script id="nf-tmpl-drawer-header-default" type="text/template">
    <header class="nf-drawer-header">
        ...
        <a href="#" title="Done" class="nf-button primary nf-close-drawer <%= renderDisabled() %>"
           tabindex="-1">Done</a>
    </header>
</script>
...
<script id="nf-tmpl-drawer-header-view-changes" type="text/template">
    <header class="nf-drawer-header">
        <div>
            <a href="#" title="Undo All" class="nf-button secondary undoChanges" style="float:left;" tabindex="-1"><span
                    class="dashicons dashicons-backup"></span> Undo All</a>
        </div>
...
<script id="nf-tmpl-drawer-content-new-form" type="text/template">
    ...
    <div>
        <a href="#" title="Not Yet" class="nf-button secondary nf-close-drawer" style="float:left;" tabindex="-1">Not
            Yet</a>
    </div>
...
<script id="nf-tmpl-app-menu-item" type="text/template">
    <li><a href="<%= renderUrl() %>" title="<%= nicename %>" class="<%= renderClasses() %>"
           target="<%= renderTarget() %>" <%= renderDisabled() %>><span class="app-menu-text"><%= nicename %></span><%=
        renderDashicons() %></a></li>
</script>
...
<script id="nf-tmpl-edit-setting-number" type="text/template">
    <label for="<%= name %>"><%= label %> <%= renderTooltip() %>
        <input type="number" class="setting" id="<%= name %>" value="<%= value %>"
               placeholder="<%= ( 'undefined' != typeof placeholder ) ? placeholder : '' %>"/>
    </label>
</script>
...
<script id="nf-tmpl-edit-setting-select" type="text/template">
    <label for="<%= name %>" class="nf-select"><%= label %> <%= renderTooltip() %>
        <select id="<%= name %>" class="setting">
            ...
            <%= ( value == option.value ) ? 'selected="selected"' : '' %>><%= option.label %></option>
            <%
            } );
            %>
        </select>
...
<script id="nf-tmpl-edit-setting-option-repeater-default-row" type="text/template">
    ...
    %>
    <div>
        <input type="checkbox" class="setting" class="nf-checkbox" <%= ( 1 == selected ) ? 'checked="checked"' : '' %>
        value="1" data-id="selected">
    </div>
    <%
    }
    %>
