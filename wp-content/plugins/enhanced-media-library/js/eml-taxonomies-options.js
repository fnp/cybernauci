(function ($) {
  ...
  $(document).on('click', 'li .wpuxss-eml-button-remove', function () {
    ...
    if (target.hasClass('wpuxss-eml-clone-taxonomy')) {
      ...
      emlConfirmDialog(l10n.tax_deletion_confirm_title,
        l10n.tax_deletion_confirm_text_p1 + l10n.tax_deletion_confirm_text_p2 + l10n.tax_deletion_confirm_text_p3 + l10n.tax_deletion_confirm_text_p4,
        l10n.tax_deletion_yes, l10n.cancel, 'button button-primary eml-warning-button')
        .done(function () {
  ...
  $(document).on('click', '.wpuxss-eml-button-create-taxonomy', function () {
    $('.wpuxss-eml-media-taxonomy-list').find('.wpuxss-eml-clone').clone().attr('class',
      'wpuxss-eml-clone-taxonomy').appendTo('.wpuxss-eml-media-taxonomy-list').show(300);
  ...
  $(document).on('blur', '.wpuxss-eml-clone-taxonomy .wpuxss-eml-singular_name', function () {
    ...
    if ('' !== taxonomy_name) {
      ...
      if ($(
          '.wpuxss-eml-clone-taxonomy[id=' + taxonomy_name + '], .wpuxss-eml-taxonomy[id=' + taxonomy_name + '], .wpuxss-non-eml-taxonomy[id=' + taxonomy_name + ']').length > 1 || -1 !== $.inArray(
          taxonomy_name, built_in)) {
        emlAlertDialog(l10n.tax_error_duplicate_title, l10n.tax_error_duplicate_text, l10n.okay,
      ...
      $(this).closest('.wpuxss-eml-clone-taxonomy').find('.wpuxss-eml-assigned').attr('name',
        'wpuxss_eml_taxonomies[' + taxonomy_name + '][assigned]');
      $(this).closest('.wpuxss-eml-clone-taxonomy').find('.wpuxss-eml-eml_media').attr('name',
        'wpuxss_eml_taxonomies[' + taxonomy_name + '][eml_media]');
      $(this).closest('.wpuxss-eml-clone-taxonomy').find('.wpuxss-eml-create_taxonomy').attr('name',
        'wpuxss_eml_taxonomies[' + taxonomy_name + '][create_taxonomy]');
  ...
  function addTaxonomyName(edit_box, taxonomy_name) {
    ...
    var fields = {
      'labels': ['name', 'menu_name', 'all_items', 'edit_item', 'view_item', 'update_item', 'add_new_item',
                 'new_item_name', 'parent_item', 'search_items'],
    ...
    $.each(fields, function (index, field) {
      ...
      if (index === field) {
        edit_box.find('.wpuxss-eml-' + field).attr('name',
          'wpuxss_eml_taxonomies[' + taxonomy_name + '][' + field + ']');
      }
      else {
        $.each(field, function (i, field) {
          edit_box.find('.wpuxss-eml-' + field).attr('name',
            'wpuxss_eml_taxonomies[' + taxonomy_name + '][' + index + '][' + field + ']');
        });
      }
    });
  }
  ...
  $(document).on('blur', '.wpuxss-eml-clone-taxonomy .wpuxss-eml-name, .wpuxss-eml-taxonomy .wpuxss-eml-name',
    function () {
    ...
    $('.wpuxss-eml-clone-taxonomy, .wpuxss-eml-taxonomy').each(function (index) {
      ...
      if (!$('.wpuxss-eml-singular_name', this).val() && !$('.wpuxss-eml-name', this).val()) {
        ...
        alert_text = l10n.tax_error_empty_plural;
          ...
          '.wpuxss-eml-clone-taxonomy[id=' + current_taxonomy + '], .wpuxss-eml-taxonomy[id=' + current_taxonomy + '], .wpuxss-non-eml-taxonomy[id=' + current_taxonomy + ']').length > 1 || -1 !== $.inArray(
          current_taxonomy, built_in)) {
        submit_it = false;
  ...
  $(document).on('click', '.eml-button-synchronize-terms', function (event) {
    ...
    emlConfirmDialog(l10n.sync_warning_title, l10n.sync_warning_text, l10n.sync_warning_yes, l10n.sync_warning_no,
      'button button-primary')
      .done(function () {
