(function ($, _) {
  ...
  $(document).on('click', '.wpuxss-eml-button-create-mime', function () {
    ...
    $('.wpuxss-eml-mime-type-list').find('.wpuxss-eml-clone').clone().attr('class', 'wpuxss-eml-clone-mime').prependTo(
      '.wpuxss-eml-mime-type-list tbody').show(300).find('input').first().focus();
  ...
  $(document).on('click', '#eml-restore-mime-types-settings', function (event) {
    ...
    emlConfirmDialog(l10n.mime_restoring_confirm_title, l10n.mime_restoring_confirm_text, l10n.mime_restoring_yes,
      l10n.cancel, 'button button-primary eml-warning-button')
      .done(function () {
  ...
  $('#wpuxss-eml-form-mimetypes').submit(function (event) {
    ...
    $('.wpuxss-eml-clone-mime').each(function (index) {
      ...
      if (!$('.wpuxss-eml-type', this).val() || $('.wpuxss-eml-type', this).val() == '' || !$('.wpuxss-eml-mime',
          this).val() || $('.wpuxss-eml-mime', this).val() == '') {
